...
C4Container
  ...
  Container(compat, "PlatformCompat", "Swift file (Sources/App)", "Cross-platform shim: monoTimeNanoseconds(), setTermiosCCDefaults(), platformWrite().")
...
- `setTermiosCCDefaults(_:)` — sets `c_cc[VMIN]` and `c_cc[VTIME]` using `withUnsafeMutablePointer(to: &raw.c_cc)` and the platform-provided `VMIN`/`VTIME` constants. Avoids hardcoded tuple indices.
- `platformWrite(_:_:_:)` — wraps the looping `write()` call to stdout, ensuring correct behaviour on both platforms.

`PlatformCompat.swift` is the designated location for ALL current and future platform-specific abstractions in the `DCJam2026` target.

### Modified Component: `Sources/App/ANSITerminal.swift`

Changes:
1. Replaced `import Darwin` with `#if canImport(Darwin)` / `#elseif canImport(Glibc)` conditional.
2. Replaced hardcoded `raw.c_cc.16 = 1` and `raw.c_cc.17 = 0` with `setTermiosCCDefaults()` from `PlatformCompat.swift`.
3. Replaced direct `write(...)` loop with `platformWrite()` from `PlatformCompat.swift`.

No logic changes. No protocol changes. `TUIOutputPort` boundary unaffected.

### Modified Component: `Sources/App/InputHandler.swift`

Changes:
1. Replaced `import Darwin` with `#if canImport(Darwin)` / `#elseif canImport(Glibc)` conditional.
2. Replaced `Darwin.open(...)`, `Darwin.close(...)`, `Darwin.read(...)` with unqualified calls.
...
Changes:
1. Replaced `import Darwin` with `#if canImport(Darwin)` / `#elseif canImport(Glibc)` conditional.
2. Replaced both `clock_gettime_nsec_np(CLOCK_MONOTONIC)` call sites with `monoTimeNanoseconds()` from `PlatformCompat.swift`.
3. `usleep` — no change needed (available in both Darwin and Glibc).
...
Given a full test run on macOS, when `swift test` is executed, then all existing tests pass. The same test suite passes on Linux.
...
Rules:
- `GameDomain` target has zero imports from `Sources/App/` (enforced structurally by SwiftPM)
- `PlatformCompat.swift` is the sole location for platform-conditional code — enforced by code review convention; documented in `docs/CLAUDE.md`
- No new `import Darwin` or `import Glibc` statements permitted outside `PlatformCompat.swift` — documented as a project convention

Note: Swift does not have a dedicated architecture linting tool equivalent to ArchUnit or import-linter as of April 2026. SwiftPM's module boundary enforcement (compile-time guarantee) covers the most critical rule (GameDomain isolation). Module-internal import conventions are documented in `docs/CLAUDE.md` and enforced by code review.
