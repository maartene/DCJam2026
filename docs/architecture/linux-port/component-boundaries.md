...
- Exports: `monoTimeNanoseconds() -> UInt64`; `setTermiosCCDefaults(_:)`; `platformWrite(_:_:_:)`
- Consumed by: `GameLoop.swift` (clock), `ANSITerminal.swift` (c_cc indices, write)
- Dependencies: none beyond the platform module (Darwin/Glibc) itself

**Convention**: `PlatformCompat.swift` is the only permitted location for platform-conditional imports or platform-specific symbol abstractions. Files other than `PlatformCompat.swift` use unconditional `#if canImport(...)` guards only for the top-level module import line itself.
...
- Platform boundary change: replaces `import Darwin` with conditional import; fixes `c_cc` index access using `setTermiosCCDefaults()`; delegates terminal write to `platformWrite()`
...
Sources/App/
  ...
  ANSITerminal.swift       <- conditional import line only; c_cc + write via PlatformCompat
...
DCJam2026 (executable)
  ...
  ├── PlatformCompat.swift    <- platform shim (Darwin OR Glibc)
  ├── ANSITerminal.swift      <- uses PlatformCompat for c_cc + write; uses Darwin/Glibc for POSIX calls
