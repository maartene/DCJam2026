...
### Darwin (macOS only)
- **Used by**: `ANSITerminal.swift`, `InputHandler.swift`, `GameLoop.swift` (post-change: under `#if canImport(Darwin)` guard)
- **Used by**: `PlatformCompat.swift` (Darwin-side implementations of `monoTimeNanoseconds()` etc.)

### Glibc (Linux only)
- **Used by**: same files post-change, under `#elseif canImport(Glibc)` guard
...
|----------|-----------|--------|
| macOS 14+ | Swift 6.3 | `swift build` passes; tests pass |
| Ubuntu 22.04 | Swift 6.3 | `swift build` passes; tests pass |
| Ubuntu 24.04 | Swift 6.3 | `swift build` passes; tests pass |
